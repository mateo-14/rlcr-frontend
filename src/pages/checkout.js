...
export default function Checkout() {
  ...
  const formData = useMemo(
    () => (router.query.c ? decodeB64Object(router.query.c) : null),
    [router.query.c]
  );
          ...
          (order ? (
            <>
              ...
              <p className="text-3xl text-white">
                Te enviamos un DM a tu Discord{' '}
                <span className="text-purple-500">({user.data.username})</span> con información del
                pedido.
                <br />
...
function Form({ formData, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentAccount) data.paymentAccount = paymentAccount;
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Input
        className="mb-4"
        label={`Usuario de ${formData.paymentMethodName}`}
        id="paymentAccount"
        required
      ...
      />
      <Input
        label="Tu usuario de Demo Games"
        id="account"
        required
      ...
      <p className="text-white mt-6 mb-4">
        * Al confirmar es necesario que autorices nuestra aplicación de Discord para seguir con el
        proceso de pago. (Es necesario tener una cuenta de Discord)
      </p>
